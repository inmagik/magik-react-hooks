...
describe('useQueryParam', () => {
  it('should parse a query string with "?" with not much effort', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, () => { }, 'a', 'null'))
  ...
  it('should use default value if param is not present', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, () => { }, 'e', '@null@'))
  ...
  it('should write a query string with not much effort', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, setter, 'a', 'default_value'))
  ...
  it('can accept a decoder (functional form)', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, () => { }, 'a', '123', decoder))
  ...
  it('can accept a decoder (object form)', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, () => { }, 'a', 'ciao', { decode: decoder }))
  ...
  it('does not call the decoder when returning default value', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, () => { }, 'e', 'ciao', { decode: decoder }))
  ...
  it('should accept an encoder (functional form)', () => {
    ...
    const encoder = (fromQs, toQs) => {
      if (toQs === undefined) {
        return fromQs.fromCharCode(0) - 65
      }
    ...
    const { result } = renderHook(() => useQueryParam(qs, setter, 'a', 'ciao', encoder))
  ...
  it('should accept an encoder (object form)', () => {
    ...
    const { result } = renderHook(() => useQueryParam(qs, setter, 'a', 'ciao', { encode: encoder }))
